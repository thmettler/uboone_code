...
void bernfebdaq::CRTTrackProducer::beginJob()
{
  ...
  my_tree_->Branch("x1", &x1, "x1 (cm)/D");
  my_tree_->Branch("y1", &y1, "y1 (cm)/D");
  my_tree_->Branch("z1", &z1, "z1 (cm)/D");
  my_tree_->Branch("x2", &x2, "x2 (cm)/D");
  my_tree_->Branch("y2", &y2, "y2 (cm)/D");
  my_tree_->Branch("z2", &z2, "z2 (cm)/D");
  my_tree_->Branch("plane1", &plane1, "plane1/I");
  my_tree_->Branch("plane2", &plane2, "plane2/I");
  my_tree_->Branch("track_time_s", &track_time_s, "time (s)/D");
  my_tree_->Branch("track_time_ns", &track_time_ns, "time (ns)/D");
  my_tree_->Branch("track_length", &length, "Track lenght (cm)/D");
  my_tree_->Branch("track_timdiff", &time_diff, "Time_diff (ns)/D");
  my_tree_->Branch("track_theta", &theta, "Theta_xy (º)/D");
  my_tree_->Branch("track_phi", &phi, "Phi_xy (º)/D");
