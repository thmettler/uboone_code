...
struct crt::EVENT_t{
		uint16_t mac5;  //mac address of the CRT module
		uint16_t flags; //flags for special events (e.g PPS, beam trigger)
    uint16_t lostcpu; // number of lost events in cpu
    uint16_t lostfpga;  // numberof lost events in fpga
		uint32_t ts0; // timestamp in ns related to GPS PPS
		uint32_t ts1; // timestamp in ns related to the beam trigger
		uint16_t adc[32]; // amplitude information of the photomultiliers
};
