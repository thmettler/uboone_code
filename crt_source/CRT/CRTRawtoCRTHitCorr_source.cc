...
// Version: V02_02 || added half top option ||  second assinmgent depends on poll time
...
void crt::CRTRawInputDetail::readFile(std::string const & filename, art::FileBlock*& fb)
{
  ...
  Init_mspoll_delay();  //fill the factor for poll-ms assignement
...
void crt::CRTRawInputDetail::receive_data(){
  ...
  if(IsTop_==1){
    //printf("seek: %d, event: %ld, size_ev; %ld\n", SEEK_CUR,tot_events,size_ev);
  ...
  int all_fine=0;//control number if the jump happened after a reference pulse
	int start_ms,end_ms,start_sec,end_sec;
  for(int i=0; i<received_events; i++){
   //prereassign=0;
    if(evbuf[i].mac5==0xFFFF){    //reads the spezial event
      ...
      previous_s_end=this_start_sec;
      previous_s_start=this_end_sec;
      ...
      for(int febnr=0;febnr<MAXFEBNR;febnr++){ //A loof over all FEB
        ...
        if(ev_counter_mac[febnr]>0){  //B assign ms and second to first event after poll
          ...
          while(evbuf_pro[febnr][first_newevent].sec!=0 && first_newevent<ev_counter_mac[febnr]){
            first_newevent++;
          }
          //first_newevent--;
				  int this_ms = (start_ms +(int)((end_ms-start_ms)*Ms_poll[febnr]+0.5));
          uint32_t this_ts0=evbuf_pro[febnr][first_newevent].ts0;
          ...
          for( int j=first_newevent+1; j<ev_counter_mac[febnr]; j++){  //C assign all corrected ms and sec
            ...
            if(evbuf_pro[febnr][j].ts0>this_ts0) delta_ts0 = 1e9+this_ts0-evbuf_pro[febnr][j].ts0;
...
void crt::CRTRawInputDetail::scan_buffer_filter(int mac){  //scan over all events of one plane over one sec and search all possible coincidences
  ...
  for(int i=last1_ms[mac];i<ev_counter_scan[mac];i++){
     //printf("scan-buffer: %3d: mac: %2d, flags: %2d, ts0: %10d, ts1: %10d, sec: %10d , ms: %d, dt: %d, lostcpu: %d\n", i,evbuf_scan[mac][i].mac5, evbuf_scan[mac][i].flags, evbuf_scan[mac][i].ts0,evbuf_scan[mac][i].ts1,evbuf_scan[mac][i].sec,evbuf_scan[mac][i].ms, evbuf_scan[mac][i].ts0-evbuf_scan[mac][i].ts1, evbuf_scan[mac][i].lostcpu ); 
    ...
    for(int i=0; i<ev_counter_filter[mac];i++){
      for(int j=0;j<approved_ts1ref;j++){
        if(((ts1_ref_approved[j][0]>evbuf_filter[mac][i].ts0_scaled) && (ts1_ref_approved[j][0]-evbuf_filter[mac][i].ts0_scaled)<MAX_TIME_PREBEAM) ||
          ...
          if((evbuf_filter[mac][i].ts0_scaled)<ts1_ref_approved[j][0]){
...
void crt::CRTRawInputDetail::make2DHit(){
  ...
  if( (pos_tevt1[3]==pos_st1[3] ) && (pos_tevt1[4]!=pos_st1[4]) 
    ...
    double ts1_local1=0, ts1_local2=0;
    ...
    //printf("ts1: %d, ts2 %d, flags: %d, flags: %d\n",coincidence[0].ts1,coincidence[1].ts1, coincidence[0].flags, coincidence[1].flags);
    //printf("ts11: %lf, ts12: %lf, beam1: %lf, beam2: %lf, beam: %lf\n",ts1_local1,ts1_local2,beam1_time_ns,beam2_time_ns,beam_time_ns);
