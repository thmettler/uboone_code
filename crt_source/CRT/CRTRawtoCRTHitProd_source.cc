...
// Version: V02_01 || added half top option || second assigmnent depends on full ts0 seconds || half macaddress
    ...
    fPrevSubRunNumber(0)
{
  ...
  WhichHalf_ = ps.get<int>("WhichHalf");
	IsDownStream_ = ps.get<int>("IsDownStream");
  HalfTop_= ps.get<std::string>("HalfTop_file");
}
...
void crt::CRTRawInputDetail::readFile(std::string const & filename, art::FileBlock*& fb)
{
  ...
  crt::auxfunctions::FillGain(CRTPedestals_, SiPMpedestal); //key = FEB*100+ch  //same for pedestals 
	...
	Init_HalfTop(HalfTop_,mac_half_top);
...
void crt::CRTRawInputDetail::receive_data(){
  ...
  for(int i=0; i<received_events; i++){
		proceed=1;
		//printf("proceed? %d",proceed);
		for(int z=0; z<mac_half_top[IsDownStream_][99]; z++){
			//printf("proceed? %d, mac: %d == %d",proceed,evbuf[i].mac5,mac_half_top[IsDownStream_][z]);
			if(evbuf[i].mac5==mac_half_top[IsDownStream_][z]){
				proceed=0;
				//printf("proceed? %d",proceed);
				}
			//printf("\n");
		}
		//printf("proceed? %d, mac: %d\n",proceed,evbuf[i].mac5);
...
void crt::CRTRawInputDetail::Init_HalfTop(std::string filename, int mac_buffer[3][100]){
  ...
  if (in.is_open()){
   ...
   else
   {
	...
	mac_buffer[IsDownStream_][99]=counter;
	std::cout<<"IsDownStream: " << IsDownStream_ <<std::endl;
	for(int z=0; z<mac_half_top[IsDownStream_][99]; z++){
		std::cout<<"not allowed macs: " << mac_half_top[IsDownStream_][z] << " of " << mac_half_top[IsDownStream_][99] <<std::endl;
	}
}
